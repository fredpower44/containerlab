...
var (
	// defined env vars for the ixia
	ixiaEnv = map[string]string{
...
func init() {
	nodes.Register(nodes.NodeKindIXIA, func() nodes.Node {
		return new(ixia)
	})
}
...
func (s *ixia) Init(cfg *types.NodeConfig, opts ...nodes.NodeOption) error {
	...
	s.cfg.Env = utils.MergeStringMaps(ixiaEnv, s.cfg.Env)
